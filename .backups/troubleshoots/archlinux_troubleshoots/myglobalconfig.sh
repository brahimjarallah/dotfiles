...
ln -sf /usr/share/zoneinfo/Africa/Tunisia /etc/localtime
hwclock --systohc
sed -i '177s/.//' /etc/locale.gen
locale-gen
echo "LANG=en_US.UTF-8" >> /etc/locale.conf
echo "KEYMAP=tn_CH-latin1" >> /etc/vconsole.conf
...
useradd -m brahim
echo brahim:password | chpasswd
usermod -aG libvirt brahim

echo "brahim ALL=(ALL) ALL" >> /etc/sudoers.d/brahim
